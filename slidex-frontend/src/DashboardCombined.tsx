...
export default function DashboardCombined() {
    ...
    return (
      ...
      `}</style>
            ...
            {/* 🚨 ELEGANCKI, WIELKI BANER BŁĘDU / BLOKADY TORU */}
            {alarm && (
                ...
                <header className={`mb-3 lg:mb-5 border-b pb-3 flex items-center justify-between gap-3 w-full ${theme.headerBorder}`}>
                    ...
                    <div className="flex items-center gap-2 lg:gap-3">
                        ...
                        <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border ${theme.cardBg} ${theme.cardBorder}`}>
                            ...
                            <span className={`text-xs font-mono font-black ${theme.textPrimary}`}>{isConnected ? 'LIVE' : 'OFFLINE'}</span>
                        </div>
                    </div>
                </header>
                    ...
                    <div className="flex-1 h-full">
                        {highlightedPartNumber ? (
                            ...
                            <div className="w-full h-full bg-bw-cyan text-slate-950 font-black text-xs sm:text-sm lg:text-base px-4 rounded-xl border-2 border-white flex items-center justify-between shadow-[0_0_25px_rgba(46,250,217,0.4)] animate-zoom-in">
                                <div className="flex items-center gap-3 truncate">
                                    {actionType === 'OUT' ? <LogOut className="w-5 h-5 shrink-0" /> : <LogIn className="w-5 h-5 shrink-0" />}
                                    <span className="flex items-center gap-2">
                                        {actionType === 'OUT' ? 'WYDANIE P/N: ' : 'ZAŁADUNEK P/N: '}
                                        <strong className="font-mono text-base lg:text-lg underline tracking-tight">{selectedPartNumber}</strong>
                                    </span>
                                    <span className="font-mono bg-slate-950 text-bw-cyan px-3 py-1 rounded-lg text-xs ml-2 truncate font-bold">
                                        {targetLane ? `TOR TARGET: ${activeRack.code} • S${targetLane.shelf}-C${targetLane.column} (${targetLane.code})` : 'Brak wolnego miejsca'}
                                    </span>
                                </div>
                                <span className="text-xs bg-slate-950 text-bw-cyan px-2.5 py-1 rounded-lg uppercase font-mono font-black tracking-wider">
                                    {actionType === 'OUT' ? 'SCAN-OUT' : 'SCAN-IN'}
                                </span>
                            </div>
                        ) : (
                            <div className={`w-full h-full border rounded-xl flex items-center px-4 justify-between transition-colors duration-300 ${theme.cardBg} ${theme.cardBorder}`}>
                                <div className="flex items-center gap-3">
                                    <RefreshCw className="w-5 h-5 text-bw-cyan animate-spin" style={{ animationDuration: '10s' }} />
                                    <div>
                                        <h2 className={`text-sm lg:text-base font-black leading-tight ${theme.textPrimary}`}>SYSTEM KONTROLI LOGISTYKI SLIDEX</h2>
                                ...
                                <span className={`text-xs font-mono font-bold tracking-wider ${theme.textSecondary}`}>GOTOWY DO SKANOWANIA</span>
                            </div>
                        )}
                    </div>
                                ...
                                <div className={`flex items-center justify-between mb-4 pb-3 border-b ${theme.cardBorder}`}>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <span className={`text-xs font-mono font-bold block ${theme.textSecondary}`}>Siatka lokacyjna torów</span>
                                        </div>
                                    </div>
                                    <button
                                        onClick={() => setSelectedRackCode('')}
            ...
            {/* 🚀 EFEKTOWNA STOPKA ZGODNA Z WIDOKIEM CONFIG I MOTYWAMI */}
            <footer className={`mt-6 pt-3 border-t text-center text-[10px] font-mono ${theme.headerBorder} ${theme.textSecondary}`}>
                SLIDEX SYSTEM UNIFIED DASHBOARD MODULE v1.0 | Developed by: Bartosz Strzemieczny (strzemieczny@borgwarner.com)
            </footer>
        </div>
    );
}
